...
class Film(Model):
    ...
    def html_link(self):
        if self.FilmGroup and self.ReleaseYear: return "<a href=\"%s\">%s</a> (%i, <a href=\"%s\">%s</a>)" % (reverse('filmwatchcount:film_detail',args=[self.id]),self.Name,self.ReleaseYear,reverse('filmwatchcount:filmgroup_detail',args=[self.FilmGroup_id]),self.FilmGroup.Name)
        elif self.FilmGroup: return "<a href=\"%s\">%s</a> (<a href=\"%s\">%s</a>)" % (reverse('filmwatchcount:film_detail',args=[self.id]),self.Name,reverse('filmwatchcount:filmgroup_detail',args=[self.FilmGroup_id]),self.FilmGroup.Name)
        elif self.ReleaseYear: return "<a href=\"%s\">%s</a> (%i)" % (reverse('filmwatchcount:film_detail',args=[self.id]),self.Name,self.ReleaseYear)
        else: return "<a href=\"%s\">%s</a>" % (reverse('filmwatchcount:film_detail',args=[self.id]),self.Name)
    class Meta:
